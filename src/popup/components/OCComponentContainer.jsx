...
import {requestLocalComponent,requestComponent,editParameter,reload,apply} from '../actions';
...
class OCComponentContainer extends Component {
  ...
  componentDidMount() {
    ...
    if (metadata == null) {
      dispatch(requestComponent(baseHref + version + '/~info', false));
    }
  }
  ...
  switchToLocal() {
    ...
    return () => {
      this.context.router.push(`/component/${this.props.name}/local`);
      dispatch(requestLocalComponent(name))
    }
  }
...
function mapStateToProps(state, ownProps) {
  ...
  return {
    ...
    registryHref: stateObject[name].registryHref,
    repository: stateObject[name].repository,
    version: version,
